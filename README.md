...
```sh
docker run --name mypris --detach --publish 8000:8000 ghcr.io/no42-org/pris:latest
```
...
services:
  pris:
    ...
    image: ghcr.io/no42-org/pris:latest
    environment:
      - TZ=Europe/Berlin
    volumes:
...
- [OpenJDK] or another JDK 21 (`java`, `javac`) — the build targets Java 21 bytecode; the release container bundles a JDK 25 runtime
- Apache [Maven] (`mvn`)
